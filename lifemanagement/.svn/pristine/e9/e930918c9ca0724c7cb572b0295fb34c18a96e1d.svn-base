...
//allow db deletes (assumes first column is the primary key of the table)
$option_db_allow_delete=1;
...
if($_POST['ajax'])
{
  ...
  case 'updcell':
    ...
    */
    ...
    case 'delrow':
    /*
        AJAX 'delrow' request variables:
        $ajax:  the ajax command ('delrow')
        $tbl:   table name
        $rowid: row id 
        $colid: columnname
        $new:   the new cell value that was typed into the text input
    ...
    */
    ...
    if($option_db_allow_delete)
    {
      //NOTE: assumes first column is the primary key of the table
      $col_rowid=mysql_field_name(mysql_query("SELECT * FROM `$tbl` LIMIT 1"),0);
      $sql = "Delete from `$tbl` WHERE `$col_rowid`='$rowid' LIMIT 1";
     
      if(!mysql_query($sql)) 
        echo mysql_error() . " in $sql"; 
      elseif(mysql_affected_rows()==0)
        echo "Database table delete failed.";
    }
    else
    {
      echo 'Database deletes are disabled in example_mysql.php. set $option_db_allow_update to enable.';
    }
...
function BuildTable($db,$tbl,$pos=1,$sortcol='',$sortdesc=0)
{
  ...
  $user=$_SESSION['myusername'];
	...
	$headerattrib = array('style' => 'background-color: skyblue; cursor:pointer; text-align: left; padding-left: 5px; padding-right: 5px;');

  //get data and rowcount
  $possql=$pos-1;
  if($sortcol) $orderby=" ORDER BY $sortcol ".($sortdesc?'DESC':'');
  $result = mysql_query("SELECT id, name, type, description, priority, duedate, actionable, nextaction, project, status FROM $tbl where memberid = '{$user}'  ORDER BY duedate ASC LIMIT $possql,$pospage");
  ...
  $at->AddColumnText ("name","Task Name");
  $at->AddColumnSelect   ("type","Type",array("Daily","Weekly","Monthly","Yearly","Project Specific"));
  $at->AddColumnTextarea     ("description","Description",200,100);
  $at->AddColumnSelect   ("priority","Priority",array("Low","Medium","High"));
  $at->AddColumnText("duedate","Due Date");
  $at->AddColumnText ("actionable","Actionable");
  $at->AddColumnTextarea ("nextaction","Next Action",200,100);
  $at->AddColumnText ("project","Project");
  $at->AddColumnSelect ("status","Status", array("Not Started","In Progress","Complete","Blocked"));
  $at->AddDeleteCheckBox("delete","Delete");
